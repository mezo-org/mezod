...
async function updateAddresses(env: Env) {
  ...
  const stmts = addresses.map((item) => {
    return env.DB.prepare(
      `INSERT OR IGNORE INTO activity (address, tx_count, claimed_btc, deployer, updated_at) VALUES (?1, ?2, ?3, ?4, ?5)`,
...
async function updateActivity(env: Env) {
  ...
  const stmts = activity.map((item) => {
    return env.DB.prepare(
      `UPDATE activity SET tx_count = ?1, claimed_btc = ?2, updated_at = datetime('now') WHERE address = ?3`,
...
async function batchExecuteStmts(
  env: Env,
...
async function getAddresses(env: Env, limit: number): Promise<string[]> {
  ...
  const { results } = await env.DB.prepare(
    `SELECT address FROM activity ORDER BY updated_at ASC LIMIT ${limit}`,
  ).all<{ address: string }>()
...
async function fetchAddresses(env: Env): Promise<AddressItem[]> {
  const bsAPI = new BlockScoutAPI(env.BLOCKSCOUT_API_URL, env.DB)
  const EOAs = await bsAPI.externallyOwnedAccounts()
  const contracts = await bsAPI.contracts()
  return [...EOAs, ...contracts].filter(
    (item) => item.address !== env.FAUCET_ADDRESS,
  )
}

async function fetchActivity(
  env: Env,
): Promise<Pick<ActivityItem, "address" | "txCount" | "claimedBTC">[]> {
  const bsAPI = new BlockScoutAPI(env.BLOCKSCOUT_API_URL, env.DB)
  const txsFromFaucet = await bsAPI.txs(env.FAUCET_ADDRESS, "from")

  const addressToClaimedBTC = txsFromFaucet.reduce((acc, tx) => {
    if (!acc[tx.to]) {
      acc[tx.to] = BigInt(0)
    }
  ...
  console.log(
    `fetched activity for ${activity.length}/${addressBatch.length} addresses in the batch`,
  )
