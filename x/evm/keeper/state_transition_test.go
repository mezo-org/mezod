...
func (suite *KeeperTestSuite) TestGetHashFn() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			uint64(suite.ctx.BlockHeight()), //nolint:gosec
			func() {
...
func (suite *KeeperTestSuite) TestApplyMessage() {
	...
	proposerAddress := suite.ctx.BlockHeader().ProposerAddress
	config, err := suite.app.EvmKeeper.EVMConfig(suite.ctx, proposerAddress, big.NewInt(31611))
	suite.Require().NoError(err)
	...
	res, _, err := suite.app.EvmKeeper.ApplyMessage(suite.ctx, msg, tracer, true)
...
func (suite *KeeperTestSuite) TestApplyMessageWithConfig() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			tc.malleate()
			res, _, err := suite.app.EvmKeeper.ApplyMessageWithConfig(suite.ctx, keeper.WrapMessage(msg), nil, true, config, txConfig)
