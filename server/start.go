...
`,
	...
	cmd.Flags().Bool(srvflags.EnableTestbedPrecompile, false, "Enable the testbed precompile; Compatible only with local networks, incompatible with mainnet and testnet")
	...
	cmd.Flags().String(server.FlagMinGasPrices, "", "Minimum gas prices to accept for transactions; Any fee in a tx must meet this minimum (e.g. 20000000000abtc)") //nolint:lll
	...
	cmd.Flags().Uint64(srvflags.JSONRPCGasCap, config.DefaultGasCap, "Sets a cap on gas that can be used in eth_call/estimateGas unit is abtc (0=infinite)")      //nolint:lll
	cmd.Flags().Float64(srvflags.JSONRPCTxFeeCap, config.DefaultTxFeeCap, "Sets a cap on transaction fee that can be sent via the RPC APIs (1 = default 1 mezo)") //nolint:lll
	...
	cmd.Flags().String(srvflags.BridgeOutServerAddress, config.DefaultBridgeOutServerAddress, "Address of the server to listen on for requests for `AssetsUnlocked` entries from the Ethereum sidecar")
