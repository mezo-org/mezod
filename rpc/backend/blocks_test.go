...
func (suite *BackendTestSuite) TestGetBlockByNumber() {
	...
	testCases := []struct {
		...
		},
		{
			...
			nil,
			nil,
			func(blockNum ethrpc.BlockNumber, _ sdkmath.Int, _ sdk.AccAddress, txBz []byte) {
				height := blockNum.Int64()
				client := suite.backend.clientCtx.Client.(*mocks.Client)
...
func (suite *BackendTestSuite) TestGetBlockByHash() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			nil,
			nil,
			func(hash common.Hash, _ sdkmath.Int, _ sdk.AccAddress, txBz []byte) {
				client := suite.backend.clientCtx.Client.(*mocks.Client)
...
func (suite *BackendTestSuite) TestGetEthBlockFromTendermint() {
	...
	testCases := []struct {
		...
		},
		{
			...
			true,
			func(_ sdkmath.Int, validator sdk.AccAddress, height int64) {
				queryClient := suite.backend.queryClient.QueryClient.(*mocks.EVMQueryClient)
		...
		},
		{
			...
			true,
			func(baseFee sdkmath.Int, _ sdk.AccAddress, height int64) {
				queryClient := suite.backend.queryClient.QueryClient.(*mocks.EVMQueryClient)
				RegisterBaseFee(queryClient, baseFee)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			gasUsed := new(big.Int).SetUint64(uint64(tc.blockRes.TxsResults[0].GasUsed)) //nolint:gosec
			...
			if tc.expTxs {
				if tc.fullTx {
					rpcTx, err := ethrpc.NewRPCTransaction(
						...
						uint64(header.Height), //nolint:gosec
						uint64(0),
						tc.baseFee,
...
func (suite *BackendTestSuite) TestGetEthBlockFromTendermint_PsuedoTx() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			gasLimit := int64(^uint32(0))                                             // for `MaxGas = -1` (DefaultConsensusParams)
			gasUsed := new(big.Int).SetUint64(uint64(blockRes.TxsResults[0].GasUsed)) //nolint:gosec
...
func (suite *BackendTestSuite) TestHeaderByNumber() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			ethrpc.BlockNumber(1),
			sdkmath.NewInt(1).BigInt(),
			func(blockNum ethrpc.BlockNumber, _ sdkmath.Int) {
				height := blockNum.Int64()
				client := suite.backend.clientCtx.Client.(*mocks.Client)
				RegisterBlockError(client, height)
			},
...
func (suite *BackendTestSuite) TestHeaderByHash() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			common.BytesToHash(block.Hash()),
			sdkmath.NewInt(1).BigInt(),
			func(hash common.Hash, _ sdkmath.Int) {
				client := suite.backend.clientCtx.Client.(*mocks.Client)
				RegisterBlockByHashError(client, hash, bz)
			},
			false,
...
func (suite *BackendTestSuite) TestEthBlockFromTendermintBlock() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(baseFee sdkmath.Int, _ int64) {
				queryClient := suite.backend.queryClient.QueryClient.(*mocks.EVMQueryClient)
				RegisterBaseFee(queryClient, baseFee)
			},
			ethtypes.NewBlock(
