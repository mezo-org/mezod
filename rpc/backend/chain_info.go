...
func (b *Backend) PendingTransactions() ([]*sdk.Tx, error) {
	mempoolClient, ok := b.clientCtx.Client.(tmrpcclient.MempoolClient)
	if !ok {
		return nil, errors.New("unexpected RPC client type")
	}

	res, err := mempoolClient.UnconfirmedTxs(b.ctx, nil)
	if err != nil {
		return nil, err
	}
...
func (b *Backend) FeeHistory(
	...
	blockEnd := int64(lastBlock) //nolint:gosec // checked for int overflow already
	...
	if blockEnd < 0 {
		...
		blockEnd = int64(blockNumber) //nolint:gosec // checked for int overflow already
	}

	blocks := int64(userBlockCount)                     //nolint:gosec // checked for int overflow already
	maxBlockCount := int64(b.cfg.JSONRPC.FeeHistoryCap) //nolint:gosec // checked for int overflow already
	...
	for blockID := blockStart; blockID <= blockEnd; blockID++ {
		index := int32(blockID - blockStart) //nolint:gosec
		// tendermint block
