...
task('btcToken:name', 'Returns the name of the token', async (taskArguments, hre, runSuper) => {
  const btctoken = new hre.ethers.Contract(precompileAddress, abi, hre.ethers.provider)
  const name = await btctoken.name()
  console.log(name)
})

task('btcToken:symbol', 'Returns the symbol of the token', async (taskArguments, hre, runSuper) => {
  const btctoken = new hre.ethers.Contract(precompileAddress, abi, hre.ethers.provider)
  const symbol = await btctoken.symbol()
  console.log(symbol)
})

task('btcToken:decimals', 'Returns the decimal places of the token', async (taskArguments, hre, runSuper) => {
  const btctoken = new hre.ethers.Contract(precompileAddress, abi, hre.ethers.provider)
  const decimals = await btctoken.decimals()
  console.log(decimals)
})

task('btcToken:totalSupply', 'Returns the total number of tokens in existence', async (taskArguments, hre, runSuper) => {
  const btctoken = new hre.ethers.Contract(precompileAddress, abi, hre.ethers.provider)
