 ...
 * | duplicated authorization silently skipped                           | one authority; same authorization twice                                   | sponsor sends type-4 tx with that authorization listed twice                       | receipt 0x1; code(authority)=designator(T1); authority.nonce=1 (second copy hit nonce-mismatch, dropped)                                                      |
 * | wrong chain id silently skipped                                     | authorization signed with chainId+1                                       | sponsor sends type-4 tx with that authorization                                    | receipt 0x1; code(authority) empty; authority.nonce=0 (validate-time reject, no bump)                                                                         |
 * | chainId = 0 (any-chain) accepted                                    | authorization signed with chainId = 0                                     | sponsor sends type-4 tx with that authorization                                    | receipt 0x1; code(authority)=designator(T1)                                                                                                                   |
 * | corrupted-signature authorization silently skipped                  | authorization's r field corrupted                                         | sponsor sends type-4 tx with that authorization                                    | receipt 0x1; code(authority) empty; authority.nonce=0 (validate-time reject, no bump)                                                                         |
 * | delegated authority sends type-2 tx (EIP-3607 sender exempt)        | authority delegated to T1; funded                                         | authority sends type-2 tx setSlot(99, 123) to itself                               | receipt 0x1; receipt.from=authority; authority.nonce=2 (install + envelope); authority.slot99=123                                                             |
 * | resolves one hop from contract CALL / STATICCALL / DELEGATECALL     | authority delegated to T1                                                 | caller.callInto / staticCallInto / delegateCallInto on authority                   | CALL: authority.slot55=77, T1.slot55=0; STATICCALL returns 77; DELEGATECALL: caller.slot88=99, authority.slot88=0                                             |
 * | EXTCODE* observe 23-byte delegation designator                      | authority delegated to T1                                                 | Eip7702ExtCodeReader.sizeOf / copyOf / hashOf on authority                         | size=23; copy=designator(T1); hash=keccak256(designator(T1)) != keccak256(code(T1))                                                                           |
 * | intrinsic CallNewAccountGas (25000) per fresh authorization         | 32-SSTORE body; fresh authorities only                                    | type-4 txs with 1 vs 2 authorizations (each from a fresh authority)                | gasUsed(2 authorizations) - gasUsed(1 authorization) = 25000 exactly                                                                                          |
 * | EIP-2929 cold->warm CALL into delegated authority                   | authority delegated to T1                                                 | Eip7702Caller.callTwiceMeasured: 2 CALLs to authority, tick() body                 | gasSecond < gasFirst; gasFirst - gasSecond in [4800, 5200] ~= 2 x COLD_EXTRA (authority + T1)                                                                 |
 */
